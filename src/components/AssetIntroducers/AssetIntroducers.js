...
class AssetIntroducers extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      countryData: null,
      mapData: [
        ['Country', 'Purchased NFTs', 'Available NFTs'],
        ['', 0, 0], // used to make the map load immediately before the network request is done
      ],
      lockedDMG: null,
    };
  }
    ...
    fetch(`https://api.defimoneymarket.com/v1/asset-introducers/all`)
      ...
      .then(countries => {
        ...
        for (let country in countries) {
          ...
          const available_affiliates = countries[country]['affiliates'].filter(
            affiliate => !affiliate['is_on_secondary_market'],
          ).length;
          const available_principals = countries[country]['principals'].filter(
            affiliate => !affiliate['is_on_secondary_market'],
          ).length;
          const total_available = (available_affiliates || 0) + (available_principals || 0);
          const purchased_affiliates = countries[country]['affiliates'].filter(
            affiliate => affiliate['is_on_secondary_market'],
          ).length;
          const purchased_principals = countries[country]['principals'].filter(
            affiliate => affiliate['is_on_secondary_market'],
          ).length;
        ...
        if (constructedCountryData) {
          for (let country in constructedCountryData) {
            mapData.push([
              constructedCountryData[country].country,
              constructedCountryData[country].totalPurchased,
              constructedCountryData[country].totalAvailable,
            ]);
          }
        }
    ...
    return (
      <div className={styles.assetIntroducersWrapper}>
        <div className={styles.pageTitleWrapper}>
          <div className={styles.pageTitle}>Asset Introducers by Country</div>
        ...
        {this.state.lockedDMG && (
          <div className={styles.totalWrapper}>
            <div className={styles.totalTitle}>Total amount of DMG locked in asset introducer NFTs:</div>
        ...
        <div className={styles.purchaseLinkWrapper}>
          <a
            href={'https://dao.defimoneymarket.com/asset-introducers/purchase'}
